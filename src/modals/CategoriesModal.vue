  ...
  <UiModal name="CategoriesModal" icon="albumsO" :title="tr.categories">
    ...
    <IonList>
      <IonItem>
        <LabelNote
          :label="tr.common"
          :note="tasks.filter((it) => it.category === 'common').length"
        />
      </IonItem>
      <IonReorderGroup :disabled="false" @ionItemReorder="onReorder">
        <IonItem v-for="val in categories.slice(2)" :key="val">
          <LabelNote
            ...
            :note="tasks.filter((it) => it.category === val).length"
          />
          <IconBtn
            color="primary"
            size="small"
            icon="pencilO"
            @click="renameCategory(val)"
          />
          <IconBtn
            color="danger"
            size="small"
            icon="trashO"
            @click="deleteCategory(val)"
          />
          <IonReorder
            slot="end"
            :style="
              categories.slice(2).length === 1 ? 'pointer-events: none' : ''
            "
          />
        </IonItem>
      </IonReorderGroup>
...
const saveCategories = () =>
  storage.set('categories', JSON.stringify(categories.value));
...
const deleteCategory = async (_category) => {
  ...
  const categoryTasksSize = tasks.filter(
    (it) => it.category === _category
  ).length;
...
onMounted(async () => {
  categories.value =
    JSON.parse(await storage.get('categories')) ?? baseCategories;
