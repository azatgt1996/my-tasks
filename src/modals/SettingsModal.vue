<template>
  ...
  >
    <template #button>
      <IconBtn
        icon="saveO"
        :disabled="isEqual($params, params)"
        @click="saveParams"
      />
    </template>
    <IonList>
      ...
      <ToggleIconItem
        icon="volumeMediumO"
        :label="tr.sound"
        v-model="$params.sound"
      />
      <ToggleIconItem
        icon="alertCO"
        :label="tr.offToastAlerts"
        v-model="$params.offToastAlerts"
      />
      <ToggleIconItem
        icon="searchCO"
        :label="tr.searchInDesc"
        v-model="$params.searchInDesc"
      />
      <ToggleIconItem
        icon="returnUpBackO"
        :label="tr.autoClose"
        v-model="$params.autoClose"
      />
      <ToggleIconItem
        icon="swapVerticalO"
        :label="tr.orderByDesc"
        v-model="$params.orderByDesc"
      />
      <IonItem>
      ...
      <IconText
        icon="trashBinO"
        :text="tr.deleteAllCompleted"
        color="danger"
        :disabled="!tasks.filter((it) => it.completed).length"
        @click="$bus.emit('deleteAllCompleted')"
      />
    </IonList>
  </UiModal>
</template>
...
watch(params, (val) => storage.set('params', JSON.stringify(val)), {
  deep: true,
});
