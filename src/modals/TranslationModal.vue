<template>
  <UiModal
    ...
    icon="languageO"
    :title="`${tr.translation}: ${filled}/${all}`"
  >
    <template #button>
    ...
    <IonList lines="none" class="tr-list">
      ...
      <IonItem v-for="key in Object.keys(Translations.EN).slice(3)">
        <IonInput
          :value="Translations[lang][key].replaceAll('%s', ' ')"
          readonly
          fill="outline"
        />
        <IonInput
...
const filled = computed(
  () => Object.values(trData.value).filter((it) => it?.trim()).length
);
...
const sendTranslation = async () => {
  ...
  const trText = JSON.stringify(trData.value, null, 2).replace(
    /"([^"]+)":/g,
    '$1:'
  );
