  ...
  <IonPage id="main-content">
    ...
    <IonContent>
      ...
      <SlidableCheckList
        v-model="selected"
        :data="filtered"
        :withVibro="params.vibro"
        rightIcon="trashO"
        :leftIcon="(task) => (task.completed ? 'arrowUndoCO' : 'checkmarkCO')"
        @to-left="toggleCompleted"
        @to-right="deleteTask"
        @click-item="(task) => $bus.open('TaskModal', task)"
      >
        <template #item="task">
          <TaskItem :task />
        </template>
...
const addTask = (title) => {
  const _category =
    category.value === 'allCategories' ? 'common' : category.value;
