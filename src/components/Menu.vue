    ...
    <IonContent>
      <IonList>
        <IconText
          icon="albumsO"
          :text="tr.categories"
          @click="$bus.open('CategoriesModal')"
        />
        <IconText
          icon="mailO"
          :text="tr.contactUs"
          @click="sendToEmail('', 'Support')"
        />
        <IconText icon="shareSocialO" :text="tr.share" @click="shareApp" />
        <IconText icon="starO" :text="tr.rateApp" @click="rateApp" />
        <IconText
          icon="languageO"
          :text="tr.helpWithTranslation"
          @click="$bus.open('TranslationModal')"
        />
        <IconText
          icon="informationCO"
          :text="tr.aboutApp"
...
const appLink =
  'https://play.google.com/store/apps/details?id=com.kvarta.mytasks';
...
const showAppInfo = () => {
  const trAuthors = Object.keys(Translations).map(
    (key) => ` ${Translations[key]._language}: ${Translations[key]._trAuthor}`
  );
