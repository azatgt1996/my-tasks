...
export const IconTextBtn = ({ icon, iconPlace = 'start', text, ...props }) =>
  h(IonButton, { fill: 'clear', ...props }, () => [
    h(Ikon, { icon, slot: iconPlace }),
    text,
  ]);

export const IconText = ({ icon, text, color, ...props }) =>
  h(IonItem, { button: true, ...props }, () => [
    h(Ikon, { icon, color, class: 'mr-10' }),
    h(IonLabel, { color }, () => text),
  ]);

export const LabelNote = ({ label, note }) =>
  h(IonLabel, { class: 'shorted' }, () => [
    label,
    h(IonNote, () => ` (${note})`),
  ]);

export const MenuBtn = () =>
  h(IonButtons, { slot: 'start' }, () => h(IonMenuButton));

export const SelectOption = ({ label, ...props }) =>
  h(IonSelectOption, props, () => label);
