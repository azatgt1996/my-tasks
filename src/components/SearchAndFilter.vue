  ...
  <IonItem>
    ...
    >
      <Ikon
        ...
        color="medium"
        small
        :icon="params.searchInDesc ? 'searchCO' : 'searchS'"
      />
      <IconBtn
        slot="end"
        size="small"
...
const _filtered = computed(() => {
  ...
  return $filtered.sort((t1, t2) => {
    ...
    if (sortBy === 'created')
      return new Date(t1.created) - new Date(t2.created);
    if (sortBy === 'changed')
      return new Date(t1.changed) - new Date(t2.changed);
    if (sortBy === 'title') return t1.title.localeCompare(t2.title);
    if (sortBy === 'priority')
      return priorityNum[t1.priority] - priorityNum[t2.priority];
    if (sortBy === 'notification')
      return getDT(t1.notification) - getDT(t2.notification);
  });
});
