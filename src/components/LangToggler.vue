...
const getFlagImg = (name) =>
  new URL(`../assets/flags/${name}.png`, import.meta.url).href;

const navLang = window.navigator.language.split('-')[0].toUpperCase();
const _langs = Object.keys(Translations);
lang.value =
  localStorage.lang ?? (_langs.includes(navLang) ? navLang : _langs[0]);
...
const langClick = async () => {
  ...
  for (const opt of $$('.alert-radio-label')) {
    ...
    opt.innerHTML =
      `<img src="${flagHref}" class="flag-icon mr-10" style="width: 20px"/>` +
      Translations[_lang]._language;
  }
};
</script>
